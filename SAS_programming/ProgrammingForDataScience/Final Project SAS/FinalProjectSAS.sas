...
PROC FREQ DATA=All;
TABLE smoker*(agegroup gender educ death)/CHISQ;
WEIGHT count;
RUN;
...
LIBNAME BCSH "E:\SASProgrammingPractice\Final Project SAS";
DATA Dempts;
SET BCSH.dempts;
RUN;
...
*2e) Create an overall fracture variable (any fracture=1, non fracture=0) from ihip and ifemur;
	...
	ELSE IF ihip = 0 & ifemur = 0 THEN overallfract = 0;
...
VAR overallfrac overallpsych logstpamt logsfpamt logsppamt logsrpamt 
sqrtstpamt sqrtsfpamt sqrtsppamt sqrtsrpamt;
RUN;

PROC FREQ DATA=Master;
TABLES numtp antipsychotic;
RUN;
...
PROC MEANS DATA=Master;
VAR overallpsych logstpamt logsfpamt logsppamt logsrpamt overallfrac;
RUN;

PROC FREQ DATA=Master;
TABLE numtp antipsychotic;
RUN;
...
and function "SUBSTR" to generate a new "diabetes with ketoacidosis" indicator variable, and "PORC freq" to count how
